$(function(){
	...
	$('.catalog').each(function(index, catalog){
		...
		$last_item.append('<img class="cover_div" src="more.png" style="position:absolute; top:0;left:0;">');
	});
	...
	$('#kw').on('focus', function(){
		$(this).css({'backgroundColor': '#4C4C4C',
	...
	// icon title hover
	$('.sub').hover(function(){
		$(this).css({'transition': 'all 0s ease',
                     'background': '#00B388'}).find('.subname').css('color', 'white');
	},function(){
		$(this).css('background','').find('.subname').css({'color': '',
	...
	$('div.catalog>.sub:nth-child(8)').mouseenter(function(){
		...
		$(this).parent('div.catalog').css({'transition': 'all 0.2s ease 0.04s',
	...
	$('div.catalog').mouseleave(function(){
		...
		$(this).css({'transition': 'all 0.2s ease 0.04s',
                     'boxShadow': '',
                     'background': ''});
	});
